...
library(tidyverse)
source("/nas/longleaf/home/adaigle/DFESelfing/calculate_pi_dom75.r")
library(reshape2)
...
dfealpha_raw_results <- dfealpha_raw_results %>% 
    ...
    mutate(gamma = 0.5*(-2*Nw*Es)) %>% #MODIFIED TO ACCOUNT FOR H=0.25
...
grapes_gammazero_raw_results <- grapes_gammazero_raw_results %>% 
    ...
    select(-data, -row_number) %>%
    as.data.frame %>%
    mutate(GammaZero.negGmean = -GammaZero.negGmean*0.5) #MODIFIED TO ACCOUNT FOR H=0.25
