...
results_dir <- "/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/positive/"
source(paste0(base_dir, "scripts/calculate_pi.r"))
#source(paste0(base_dir, "scripts/calculate_alpha.r"))
alphas <- read.table( file = paste0(base_dir, "scripts/alphas1.txt"), header = TRUE)

dfe_results_dir <- "/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/positive/dfe_results/"
...
positive_grapes_plot <- ggplot(posgrapesplot, aes(x = generation, y = value, fill = factor(selfing, 
    levels = c("truth", "true0", "Adjusted DFE", "GRAPES", "true50", "50_grapes", "true80", "80_grapes", "true90", "90_grapes", "true95", "95_grapes", "true99", "99_grapes")))) +
  geom_bar(stat = "identity", position = "dodge", colour = "black") +
