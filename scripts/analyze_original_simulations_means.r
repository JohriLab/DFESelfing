...
grapes_gammazero_raw_results <- grapes_gammazero_raw_results %>% 
  group_by(DFE,selfing) %>% 
  left_join(prediction_accuracy_table) %>%
  mutate(GammaZero.negGmean=GammaZero.negGmean/(2*empircal_B_selfing_adjust*5000))
...
grapes_gammazero_simple_summary <- 
    ...
    group_by(DFE) %>%
    mutate(true_mean = unlist(true_gammas[DFE]) / (2*5000)) %>%
    mutate(true_shape = unlist(true_betas[DFE])) %>% 
...
dfealpha_raw_results_wtruth <- dfealpha_raw_results_wtruth %>% left_join(prediction_accuracy_table)
dfealpha_raw_results_wtruth2 <- dfealpha_raw_results_wtruth %>% mutate(gamma=gamma/(empircal_B_selfing_adjust*5000*2))
#now i run the class generator function on all outputs
dfealpha_raw_results_wclasses <- dfealpha_raw_results_wtruth2 %>%
...
combo_plot_with_Badjusted <- bind_rows(voodoo3,voodoo3_grapes) %>%
  ...
  filter(selfing_class != "truth") %>% 
    mutate(selfing = recode(selfing,
     ...
     'true95' = "Adjusted DFE",     
     'true99' = "Adjusted DFE")) %>%
  filter(!grepl("Adjusted DFE", selfing)) 
...
Badjusted <- ggplot(combo_plot_with_Badjusted, aes(x = generation, y = value, fill = factor(selfing, 
  ...
  geom_bar(stat = "identity", position = "dodge", colour = "black") +
  labs(x = "selection coefficient (s)", y = "proportion of mutations", fill = "") +
  geom_errorbar(aes(ymin = value - sd, ymax = value + sd), position = position_dodge(width = 0.9)) +
  expand_limits(y=c(0,1)) +
  facet_grid(rows = vars(DFE), cols = vars(selfing_class)) +
  scale_fill_manual(values = c("#404040", rep(c("#F8766D", "purple"),6))) + 
  theme(axis.text.x=element_text(size=8, angle = 30), axis.text.y=element_text(size=12), 
    axis.title.x=element_text(size=12),axis.title.y=element_text(size=12), strip.text = element_text(size=13),
    plot.title= element_text(size=0), legend.position = "bottom", legend.text = element_text(size=12)) +
  guides(fill=guide_legend(nrow=1, byrow=TRUE)) +
  scale_x_discrete(labels = c(expression("0,1E-4"),
                              expression("1E-4,1E-3"),
                              expression("1E-3,1E-2"),
                              expression("1E-2," ~ infinity))) +
...
ggsave(paste0(figures_dir, "sfigure2_new.svg"), plot = Badjusted, width = 8.5, height = 7.5, dpi = 300)
...
Badjusted_DFE1_99 <- ggplot(combo_plot_with_Badjusted_99only_DFE1, aes(x = generation, y = value, fill = factor(selfing, 
  ...
  theme(axis.text.x=element_text(size=12), axis.text.y=element_text(size=12), 
    ...
    plot.title= element_text(size=0), legend.position = "bottom", legend.text = element_text(size=12)) +
  guides(fill=guide_legend(nrow=1, byrow=TRUE)) +
  scale_x_discrete(labels = c(expression("0 - " * 10^-4),
...
saveRDS(Badjusted_DFE1_99, "/nas/longleaf/home/adaigle/DFESelfing/scripts/summary_figures/DFE1_estimate_s.rds")
saveRDS(Badjusted_DFE2_99, "/nas/longleaf/home/adaigle/DFESelfing/scripts/summary_figures/DFE2_estimate_s.rds")
saveRDS(Badjusted_DFE3_99, "/nas/longleaf/home/adaigle/DFESelfing/scripts/summary_figures/DFE3_estimate_s.rds")
...
prediction_accuracy_table2 <- dataframe_of_truth3 %>%
  left_join(dfealpha_summary2) %>%
  left_join(grapes_gammazero_summmary2) %>%
  arrange(DFE) %>%
  mutate (F = (as.numeric(selfing)/100)/(2-(as.numeric(selfing)/100)),
...
## adding adjusted truths to plots
# will just append to end now so I don't break things
F_adjusted_classes <- prediction_accuracy_table2 %>%
  mutate(output = map2(F_adjusted_gamma, true_shape, DFE_proportions_grapes)) %>%
  unnest_wider(output) %>% 
...
prediction_accuracy_table_gamma_melt <- prediction_accuracy_table2 %>% 
...
gamma_accuracy <- ggplot(prediction_accuracy_table_gamma_melt, aes(x = factor(gamma_method, levels = gammaorder), y = avg, fill = factor(gamma_method, levels = gammaorder))) +
  ...
  scale_fill_manual(values=c("#404040", rep(c("green", "lightgrey", "#F8766D", "purple"),6))) + 
...
#beta stuff
prediction_accuracy_table_beta_melt <- prediction_accuracy_table2 %>% 
