...
results_dir <- "/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/low_positive/"
source(paste0(base_dir, "scripts/calculate_pi.r"))
source(paste0(base_dir, "scripts/calculate_alpha.r"))
dfe_results_dir <- "/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/low_positive/dfe_results/"
...
# setting things up to divide mean gamma by 'B_self'
B <-tidy_summary_table %>%
    mutate(B_self = B/(1/(1+(.01 *as.numeric(selfing)) / (2-.01*as.numeric(selfing)))),
        matchname = str_c(DFE, "output", output, sep = "")) %>% 
        select(matchname, selfing, B_self)
...
joined_results <- pos_grapes_gammaexpo_raw_results %>%
    left_join(B, by = c("matchname", "selfing"))

#divide Gmean by Bself
pos_grapes_gammaexpo_raw_results_wclasses <- joined_results %>%
  mutate(output = map2(GammaExpo.negGmean/B_self, GammaExpo.negGshape, DFE_proportions_grapes)) %>%
  unnest_wider(output) %>%
...
posgrapesplot <- rbind(pos_voodoo_grapes2, df_truth_rep_self) %>%
  ...
  filter(!grepl("error", selfing_class)) %>%
    ...
    # '95_grapes' = 'GRAPES',     
    # '99_grapes' = 'GRAPES'))
posgrapesplot_noadjusted <- posgrapesplot %>% filter(selfing != "Adjusted DFE")
...
positive_grapes_plot <- ggplot(posgrapesplot_noadjusted, aes(x = generation, y = value, fill = factor(selfing, 
  ...
  scale_fill_manual(values = c("#404040", rep(c("purple"),6)),na.translate = FALSE) + 
  theme(axis.text.x=element_text(size=9, angle=30, hjust=1), axis.text.y=element_text(size=12), 
