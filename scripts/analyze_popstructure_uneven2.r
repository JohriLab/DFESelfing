...
for(Nem in Nems) {
results_dir <- paste0("/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/popstructure_uneven_2/", Nem)
source(paste0(base_dir, "scripts/calculate_pi.r"))

figures_dir <- paste0(base_dir, "figures_for_publication/")

results_dir <- paste0("/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/popstructure_uneven_2/", Nem, "/dfe_results/")

dfealpha_dir <- paste0("/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/popstructure_uneven_2/", Nem, "/dfe_results/dfealpha/")
...
#replicate DFE2output4 did not finish for 50% selfing at Nem=1, so is excluded
if (Nem == "Nem1") {
    dfealpha_output_dirs <- dfealpha_output_dirs[!grepl("DFE_alpha_output_50/DFE2output4", dfealpha_output_dirs)]
  }
...
#dfealpha_output_dirs <- dfealpha_output_dirs[!grepl("99", dfealpha_output_dirs) | !grepl("output1", dfealpha_output_dirs)]
...
grapes_dir <- paste0("/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/popstructure_uneven_2/", Nem, "/dfe_results/grapes/")
...
grapes_raw_results <- tibble(
    ...
    filter(matchname != "DFE2output4"|selfing != 50) %>% #replicate DFE2output4 did not finish for 50% selfing at Nem=1, so is excluded
    #filter(DFE!="DFE3"|selfing!=99) %>% 
    #filter(matchname != "DFE2output1"|selfing != 99) %>%
...
write.csv(combo_plot, file=paste0(base_dir, "scripts/popstructure_uneven_2_plot/", Nem, ".csv"))
}
