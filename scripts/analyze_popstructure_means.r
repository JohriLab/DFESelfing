...
#Nem <- "Nem01"
Nems <- c("Nem2", "Nem05", "Nem01", "Nem1")

for(Nem in Nems) {
results_dir <- paste0("/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/popstructure/", Nem)
source(paste0(base_dir, "scripts/calculate_pi.r"))

figures_dir <- paste0(base_dir, "figures_for_publication/")

results_dir <- paste0("/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/popstructure/", Nem, "/dfe_results/")

dfealpha_dir <- paste0("/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/popstructure/", Nem, "/dfe_results/dfealpha/")
...
grapes_dir <- paste0("/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/popstructure/", Nem, "/dfe_results/grapes/")
...
quick_summary <- prediction_accuracy_table %>% ungroup() %>%
  select(DFE,selfing, gamma, GammaZero.negGmean, b, GammaZero.negGshape) %>%
  na.omit()%>% group_by(selfing, DFE) %>% 
  summarize(across(where(is.numeric), list(avg = mean))) 

grapes_raw_results_wclasses <- prediction_accuracy_table %>%
...
mutate(output = pmap(list(empircal_B_selfing_adjust, DFE_proportions_truth))) %>%
    unnest_wider(output) %>%
    group_by(selfing, DFE) %>%
    summarize(across(where(is.numeric), list(avg = mean, sd = sd))) %>%
    rename(f0 = f0_avg) %>%  #had to rename columns bc i rerun the function
...
combo_plot <- combo_plot %>% mutate(Nem=Nem)
write.csv(combo_plot, file=paste0(base_dir, "scripts/popstructure_plot/means/", Nem, ".csv"))
}
