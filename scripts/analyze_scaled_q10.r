...
results_dir <- paste0(sim_outputs_dir, "scaling_foranalysis/")
source(paste0(base_dir, "scripts/calculate_pi_q10.r"))

figures_dir <- paste0(base_dir, "figures_for_publication/")
dfe_results_dir <- paste0(sim_outputs_dir, "scaling_foranalysis/dfe_results/")
...
dfealpha_output_dirs <- dir(path=dir(path=dfealpha_dir, full.names = T, pattern='output'), full.names=T, pattern="selected")
#dfealpha_output_dirs <- dfealpha_output_dirs[!grepl("_90/DFE1output3.txt", dfealpha_output_dirs)]
#dfealpha_output_dirs <- dfealpha_output_dirs[!grepl("_90/DFE1output4.txt", dfealpha_output_dirs)]
#dfealpha_output_dirs <- dfealpha_output_dirs[!grepl("_99/DFE1output2.txt", dfealpha_output_dirs)]
#dfealpha_output_dirs <- dfealpha_output_dirs[!grepl("output2.txt", dfealpha_output_dirs)]
#dfealpha_output_dirs <- dfealpha_output_dirs[!grepl("output3.txt", dfealpha_output_dirs)]
#dfealpha_output_dirs <- dfealpha_output_dirs[!grepl("output5.txt", dfealpha_output_dirs)]
#dfealpha_output_dirs <- dfealpha_output_dirs[!grepl("_99/DFE1output5.txt", dfealpha_output_dirs)]
...
#output_dirs <- output_dirs[!grepl("grapes_output_99", output_dirs)]
...
grapes_raw_results <- tibble(
    ...
    fullpath = list.files(path = output_dirs, pattern = ".csv$", full.names = T),
    matchname = str_extract(fullpath, "DFE\\d+output\\d+"), #will be useful for comparing results later, or combining with other programs
    selfing = str_extract(fullpath, "(?<=grapes_output_)\\d+"),
    DFE = str_extract(matchname, "(DFE)\\d+"),

) %>%
    #filter(matchname != "DFE1output3"|selfing != 90) %>%
    #filter(matchname != "DFE1output4"|selfing != 90) %>%
    #filter(matchname != "DFE1output2"|selfing != 99) %>%
    #filter(matchname != "DFE1output5"|selfing != 99) %>%
...
combo_plot_scale <- combo_plot_with_Badjusted %>% mutate(scale=10)
B_value_table_scale <- B_value_table %>% mutate(scale=10)
write.csv(combo_plot_scale, file=paste0(base_dir, "scripts/scaling/q10.csv"))
write.csv(B_value_table_scale, file=paste0(base_dir, "scripts/scaling/q10_B.csv"))
