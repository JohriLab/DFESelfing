...
dfealpha_output_dirs <- paste(paste(dfealpha_dir, dir(dfealpha_dir, pattern = "DFE_alpha_pos_output"), sep = ""),
    "/", dir(
    paste(dfealpha_dir, dir(dfealpha_dir, pattern = "DFE_alpha_pos_output"), sep = ""), pattern = "selected")
    , sep = "")
...
dfealpha_raw_results <- tibble(
    ...
    selfing = str_extract(fullpath, "(?<=DFE_alpha_pos_output_)\\d+") %>% if_else(is.na(.), "0", .), #0's were empty
...
#))
#S0DFE1:3 s50 DFE1meanS_S99
#also need to calc s80 s90 and meanS95
...
selfing_order <- c("True0", 0, 50, 99)

# Create the grouped bar chart with custom selfing order
ggplot(voodoo, aes(x = generation, y = value, fill = factor(selfing, levels = c("True0", "0", "50", "80", "90", "95", "99")))) +
  geom_bar(stat = "identity", position = "dodge", colour = "black") +
  facet_wrap(~ DFE, nrow = 2) +
...
combo <- bind_rows(voodoo, voodoo_grapes)

plot <- ggplot(combo, aes(x = generation, y = value, fill = selfing)) +
...
grapes_dir <- "/nas/longleaf/home/adaigle/DFESelfing/grapes/"
output_dirs <- paste(grapes_dir, dir(grapes_dir, pattern = "output_pos"), "/all", sep = "")
...
# Create the grouped bar chart with custom selfing order
ggplot(voodoo_grapes, aes(x = generation, y = value, fill = factor(selfing, levels = c("True0", "0", "50", "99")))) +
  geom_bar(stat = "identity", position = "dodge", colour = "black") +
  facet_wrap(~ DFE, nrow = 2) +
  labs(title = "Grapes", x = "Mutation Class (least to most deleterious)", y = "proportion of mutations", fill = "Selfing %") +
  geom_errorbar(aes(ymin = value - sd, ymax = value + sd), position = position_dodge(width = 0.9)) +
  ...
  grapes_for_plotting <- bind_rows(df_true_tidy,df_tidy,truth_tidy) %>%
    select(1:8) %>%
    melt() %>% 
    mutate(value = ifelse(is.na(value), 0, value)) %>% 
    #mutate(generation = recode(generation,
