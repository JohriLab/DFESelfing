...
dfealpha_dir <- "/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/nolinkage_h025/final_50k_outputs/dfe_results/dfealpha/"
...
grapes_dir <- "/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/nolinkage_h025/final_50k_outputs/dfe_results/grapes/"
...
quick_summary <- prediction_accuracy_table %>% ungroup() %>%
  ...
  na.omit()
write.csv(quick_summary, file="quick_summary_unlinkedh025_gammas.csv")
...
combo_plot <- bind_rows(voodoo3,voodoo3_grapes) %>%
  ...
  filter(selfing_class != "95% Selfing") %>%
    mutate(selfing = recode(selfing,
     'Dominance_adjusted_50' = 'Simulated DFE',
     'Dominance_adjusted_0' = 'Simulated DFE',
...
sfigure08 <- ggplot(combo_plot, aes(x = generation, y = value, fill = factor(selfing, 
  ...
  scale_fill_manual(values = c(c("#404040"),rep(c("#F8766D", "purple"),3))) + 
  theme(axis.text.x=element_text(size=15), axis.text.y=element_text(size=15),
