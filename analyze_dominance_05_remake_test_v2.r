...
dfealpha_dir <- "/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/dom/hdel_0_05/dfe_results/dfealpha/"
...
dfealpha_raw_results <- dfealpha_raw_results %>% 
    ...
    mutate(gamma = -0.5*2*Nw*Es) %>% #MODIFIED TO ACCOUNT FOR H=0.5
...
grapes_dir <- "/nas/longleaf/home/adaigle/work/johri_elegans/sim_outputs/dom/hdel_0_05/dfe_results/grapes/"
...
grapes_gammazero_raw_results <- grapes_gammazero_raw_results %>% 
    ...
    select(-data, -row_number) %>%
    as.data.frame %>%
    mutate(GammaZero.negGmean = -0.5*GammaZero.negGmean) #MODIFIED TO ACCOUNT FOR H=0.5
...
dataframe_of_truth3 <- dataframe_of_truth2 %>%
  ...
  mutate(dominance_gamma = true_mean * find_h(as.numeric(selfing)* (1/(1+(0.01*as.numeric(selfing)/(2-0.01*as.numeric(selfing))))), 0.5)) %>% # need selfing % and dominance
...
ggplot(combo_plot, aes(x = generation, y = value, fill = factor(selfing, 
  ...
  geom_bar(stat = "identity", position = "dodge", colour = "black") +
  labs(title = "DFEalpha and Grapes, h=0.5", x = "Mutation Class (least to most deleterious)", y = "proportion of mutations", fill = "Selfing %") +
  geom_errorbar(aes(ymin = value - sd, ymax = value + sd), position = position_dodge(width = 0.9)) +
  expand_limits(y=c(0,1)) +
  facet_grid(rows = vars(DFE), cols = vars(selfing_class)) +
...
write.csv(combo_plot, file="/nas/longleaf/home/adaigle/DFESelfing/dom_plot/h05.csv")